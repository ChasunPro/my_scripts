 ...
 * 环境变量: Fukuda
 * 变量格式: 手机号#密码
 * 多账号用 & 或 新建变量 分割，账号密码用 # 分割
 * 
 * 环境变量: PKSign
 * 变量说明: 是否执行皮卡生活签到，默认true执行，false为不执行
 */

const $ = new Env('福田e家')
const Fukuda = ($.isNode() ? process.env.Fukuda : $.getdata("Fukuda")) || '';
const PKSign = ($.isNode() ? process.env.PKSign : $.getdata("PKSign")) || 'true';
let phone = ''
let password = ''
let token = ''
let notice = ''
...
async function main() {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        try {
            ...
            if(PKSign.toLowerCase() === 'true') {
                // 皮卡生活登录
                console.log('1. 皮卡生活登录')
                ...
                if (pkLogin.code != 200) {
                    console.log(`账号登录失败: ${pkLogin.msg || '未知错误'}`)
                    continue
                }
                ...
                if (pkSign.data.integral) {
                    ...
                    console.log(`签到结果: ${pkSign.data.msg}`)
                }
            } else {
                console.log('已关闭皮卡生活签到功能')
            }
            ...
            if (login.data.signIn == "未签到") {
                ...
                if (sign && sign.data && sign.data.integral) {
                    console.log(`签到成功，获得${sign.data.integral}积分`);
                } else {
                    console.log(sign ? sign.data.msg : "签到失败，未能获取积分信息");
                }
            } else {
            ...
            console.log(`账号 ${phone} 运行出错: ${e.message || e}`)
        }
    }
    if (notice) {
