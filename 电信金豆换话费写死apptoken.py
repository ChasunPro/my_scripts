...
ss = requests.session()
ss.ssl=ssl_context
ss.headers={"User-Agent":"Mozilla/5.0 (Linux; Android 13; 22081212C Build/TKQ1.220829.002) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.97 Mobile Safari/537.36","Referer":"https://wapact.189.cn/JinDouMall/JinDouMall_independentDetails.html"}    
...
def queryInfo(phone,s):
    ...
    while a < 10:
        ...
        r = s.get('https://wapact.189.cn/gateway/golden/api/queryInfo',cookies=ck).json()
        ...
        if amountTotal< 3000:
            ...
            res = s.post('http://wapact.189.cn/gateway/stand/detail/exchange',json={"activityId":jdaid},cookies=ck).text            
...
def exchange(phone,s,title,aid, uid):
    ...
    try:
        bd = js.call('main').split('=')
        ck [bd[0]] = bd[1]
        r = s.post('https://wapact.189.cn/gateway/standExchange/detailNew/exchange',json={"activityId":aid},cookies=ck)
    ...
    for cishu in range(3):
        try:
            if rs:
                ...
                cookie = {} 
            r = s.post('https://wapact.189.cn/gateway/golden/api/lottery',json={"activityId":"6384b49b1e44396da4f1e4a3"},cookies=ck)
        except:
            pass 
        time.sleep(3)    
...
def ks(phone, ticket, uid):
    ...
    wxp[phone] = uid
    s = requests.session()
    s.headers={"User-Agent":"Mozilla/5.0 (Linux; Android 13; 22081212C Build/TKQ1.220829.002) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.97 Mobile Safari/537.36","Referer":"https://wapact.189.cn/JinDouMall/JinDouMall_independentDetails.html"}
    ...
    login = s.post('https://wapact.189.cn/unified/user/login',json={"ticket":ticket,"backUrl":"https%3A%2F%2Fwapact.189.cn%3A9001","platformCode":"P201010301","loginType":2}, cookies=ck).json()
    if login['code'] == 0:
        ...
        queryBigDataAppGetOrInfo = s.get('https://wapact.189.cn/gateway/golden/api/queryBigDataAppGetOrInfo?floorType=0&userType=1&page&1&order=2&tabOrder=',cookies=ck).json()
...
def first_request(res=''):
    global js, fw
    url = 'https://wapact.189.cn/gateway/stand/detail/exchange'
    if res == '':
        response = ss.get(url)
        res =  response.text
...
def main():
    global wt,rs
    r = ss.get('https://wapact.189.cn/gateway/standExchange/detailNew/exchange')
    if '$_ts=window' in r.text:
        rs = 1
        print("瑞数加密已开启")
        first_request()
